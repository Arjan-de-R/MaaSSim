...
def generate_vehicles_d2d(_inData, _params=None):
    ...
    vehs['registered'] = vehs.apply(lambda row: (row.registered * np.full(len(_inData.platforms.index),True)) if row.multihoming or _params.evol.drivers.regist.auto else sh_init_reg(_params, row.registered * np.full(len(_inData.platforms.index),True)), axis=1) # multi-homers, else single-homers
