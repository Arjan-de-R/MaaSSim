...
def prep_shared_rides(_inData, sp, _print=False):
    ...
    pax = _inData.passengers

    _inData.sblts.requests.shareable = pax.apply(lambda x:
                                            True if x.mode_day == 'pool'
                                            else False, axis=1)
    _inData.requests.shareable = _inData.sblts.requests.shareable
    _inData.requests.shareable = _inData.requests.shareable.fillna(False)

    if _inData.sblts.requests[_inData.sblts.requests.shareable].shape[0] == 0:  # no sharing
