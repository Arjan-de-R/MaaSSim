...
def platform_regist(inData, end_day, **kwargs):
    ...
    regist_df = pd.DataFrame(data={'inform': inData.informed, 'prev_regist': end_day.registered, 'work_exp': end_day.worked_days, 'expected_income': end_day.new_perc_inc})
    ...
    util_reg = params.evol.drivers.regist.beta * regist_df.expected_income.to_numpy() * prob_d_reg
    util_not_reg = params.evol.drivers.regist.beta * (
                inData.vehicles.res_wage.to_numpy() + params.evol.drivers.regist.cost_comp)
    ...
    # samp = np.random.rand(params.nV) <= params.evol.drivers.regist.samp   # Sample of drivers making (de)registration choice
