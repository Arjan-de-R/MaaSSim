...
def learning_unregist(inData, end_day, **kwargs):
    ...
    if (~end_day.out).any(axis=0):  # at least a single participating driver
        df['signal'] = np.random.normal(average_xp_income, params.evol.drivers.inform.signal_rel_std * average_xp_income,
                              len(inData.vehicles))
...
def platform_regist(inData, end_day, **kwargs):
    ...
    regist_df['days_since_reg'][regist_df.regist_decision] = 0
    regist_res = pd.DataFrame(data={'registered': registered, 'work_exp': regist_df.work_exp,
                                    'days_since_reg': regist_df.days_since_reg},
                              index=np.arange(1, len(inData.vehicles) + 1))
