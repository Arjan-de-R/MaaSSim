...
def add_indicators(micro, params, drivers, requests, res, all_pax):
    ...
    for item in os.listdir(dir_name): # loop through items in dir
        if item.endswith(extension): # check for ".zip" extension
            ...
            for filename in zip_ref.namelist():
                ...
                if filename == "all_pax.csv":
                    all_pax[run_id] = pd.read_csv(zip_ref.open(filename),index_col=0)
                    
            zip_ref.close() # close file
            run_id += 1
    
    return res, micro, params, drivers, requests, var_val, all_pax
